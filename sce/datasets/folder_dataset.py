...
class DatasetFolder(VisionDataset):
    ...
    Args:
        ...
        is_valid_file (callable, optional): A function that takes path of a file
            ...
            both extensions and is_valid_file should not be passed.
        class_ratio (float): ratio of classes to use if class_list None. Defaults to 1.
        sample_ratio (float): ratio of samples to use. Defaults to 1.
        class_list (Iterable[str]): If not None, List of classes to use. Defaults to None.
        seed (Optional[int]): If not None, seed used to randomly choose class and samples. Defaults to None.
    ...
    def __init__(
        ...
        assert 0 < class_ratio <= 1., "class_ratio should be comprised between 0 (excluded) and 1. (included)"
        assert 0 < sample_ratio <= 1., "sample_ratio should be comprised between 0 (excluded) and 1. (included)"

        classes, class_to_idx = self.find_classes(self.root)

        if class_ratio < 1. or sample_ratio < 1.:
            global_seed = int(os.getenv("PL_GLOBAL_SEED"))
            if global_seed is None and seed is None:
                rank_zero_warn(
                    'PL_GLOBAL_SEED environment variable is not defined aswell as the seed argument, the default seed used is 0 for class_ratio and sample_ratio.')
